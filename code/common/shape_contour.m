function [surface_x1,edges] = shape_contour(surface_x0, R)

th = 0.5;
edges = struct('X',[],'Y',[],'Z',[]);
surface_x1 = surface_x0;
Nt = size(surface_x0.TRIV,1);
lt = zeros(Nt,1);
for k=1:max(R(:)),
    
    r = double( R == k );
    DX = r(surface_x0.TRIV);
    dmin = min(DX,[],2);
    dmax = max(DX,[],2);
    ...
    edges.X = [edges.X, [v1(:,1)'; v2(:,1)']];
    edges.Y = [edges.Y, [v1(:,2)'; v2(:,2)']];
    edges.Z = [edges.Z, [v1(:,3)'; v2(:,3)']];
...
%    lt(t(i3)) = 0;
    ...
    l = [t(:)' [(Nt+1):size(surface_x1.TRIV,1)]];
    Nt = size(surface_x1.TRIV,1);
...
for k=1:size(R,2),
    ...
    Lambda = [Lambda; [lambda1(:) lambda2(:) lambda3(:)]];
    NN = [NN; nn(:)];
    TRI = [TRI; tri];
